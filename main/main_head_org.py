...
objs = bproc.loader.load_scenenet("SceneNetData/1Bedroom/bedroom_1.obj", args.scene_texture_path, label_mapping)
...
for obj in objs:
    if  isinstance(obj, bproc.types.MeshObject):
        ...
        obj.set_scale(ori_scale * 100)
       

# load head models:
head = bproc.loader.load_blend('test.blend')
#head = bproc.filter.all_with_type(head, bproc.types.MeshObject)
'''for obj in head:
    if  isinstance(obj, bproc.types.MeshObject):
        obj.set_scale([1, 1, 1])
        location = obj.get_location()
...
poi = bproc.object.compute_poi(bproc.filter.all_with_type(head, bproc.types.MeshObject))
head_location = bproc.filter.by_attr(bproc.filter.all_with_type(head, bproc.types.MeshObject), "name", "Head", regex=True)[0].get_location()
...
while tries < 10000 and poses < 1:
    ...
    tries += 1
    location = bproc.sampler.part_sphere(head_location, radius=80, part_sphere_dir_vector=[0, -1, 0], mode="SURFACE")
    # Compute rotation based on vector going from location towards poi
    _, _, _, _, hit_object, _ = bproc.object.scene_ray_cast(location, [0, 1, 0])
    if hit_object not in head:
        continue
...
while tries < 10000 and poses < 1:
    ...
    location = bproc.sampler.shell(head_location, 0.2, 1.2, elevation_min=0, elevation_max=90, azimuth_min=-180, azimuth_max=180, uniform_volume=False)
